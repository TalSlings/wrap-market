...
export default function ListingForm({
  ...
  const normalizeMaterialSource = (value: string) =>
    value
      .toLowerCase()
      .replace(/[()]/g, " ")
      .replace(/[\-_/]+/g, " ")
  ...
  const importedMaterialName = (sourceName: string) => {
    const n = normalizeMaterialSource(sourceName);

    // More specific matches must come before their parent materials.
    if (/\b(?:tussah|tussar|tussur)\b.*\bsilk\b|\bsilk\b.*\b(?:tussah|tussar|tussur)\b/.test(n)) return "משי טוסה";
    if (/\bmulberry\b.*\bsilk\b|\bsilk\b.*\bmulberry\b/.test(n)) return "משי מאלברי";
    if (/\begyptian\b.*\bcotton\b|\bcotton\b.*\begyptian\b|\bmako cotton\b/.test(n)) return "כותנה מצרית";
    if (/\b(?:pima|supima)\b.*\bcotton\b|\bcotton\b.*\b(?:pima|supima)\b/.test(n)) return "כותנת פימה";
    if (/\borganic\b.*\bcotton\b|\bcotton\b.*\borganic\b|\bgots cotton\b|\bkba cotton\b|\bbci organic.*cotton\b/.test(n)) return "כותנה אורגנית";
    if (/\bmerino\b/.test(n)) return "מרינו";
    if (/\bcashmere\b/.test(n)) return "קשמיר";
    if (/\balpaca\b/.test(n)) return "אלפקה";
    if (/\b(?:tencel|lyocell)\b/.test(n)) return "טנסל";
    if (/\bbamboo viscose\b/.test(n)) return "במבוק";
    if (/\bviscose\b/.test(n)) return "ויסקוזה";
    if (/\bbamboo\b/.test(n)) return "במבוק";
    if (/\blinen\b/.test(n)) return "פשתן";
    if (/\bhemp\b/.test(n)) return "קנבוס";
    if (/\bkapok\b/.test(n)) return "קפוק";
    if (/\bpolyester\b/.test(n)) return "פוליאסטר";
    if (/\bwool\b/.test(n)) return "צמר";
    if (/\bsilk\b/.test(n)) return "משי";
    if (/\bcotton\b/.test(n)) return "כותנה";

    return "";
  };

  const materialIdByName = (name: string) =>
    localMaterials.find(
      (m: any) =>
        m.status !== "hidden" &&
        m.is_selectable !== false &&
  ...
  const gsmKeyFromNumber = (value: number) => {
    if (!Number.isFinite(value)) return "";
    if (value <= 184) return "lte_180";
    if (value >= 345) return "gte_350";

    const rounded = Math.round(value / 10) * 10;
    const key = String(Math.max(190, Math.min(340, rounded)));
    return GSM.some(([candidate]) => candidate === key) ? key : "";
  };
  ...
  async function importFromLink() {
    ...
    if (!url) {
      setImportError("הדביקי קישור ל־WrapTrack");
      return;
    }
    ...
    try {
      const response = await fetch("/api/import-wrap", {
        method: "POST",
      ...
      if (!response.ok) {
        throw new Error(result?.error || "לא הצלחנו למלא פרטים מהקישור");
      }
      ...
      if (typeof data.manufacturer === "string" && data.manufacturer.trim()) {
        const importedName = data.manufacturer.trim();
        const existing = localManufacturers.find(
          (m: any) =>
            String(m.name).trim().toLowerCase() === importedName.toLowerCase()
        );

        if (existing) {
          setManufacturerId(existing.id);
          setNewManufacturer("");
        } else {
          setManufacturerId("");
          setNewManufacturer(importedName);
        }
        setManufacturerMsg("");
        applied += 1;
      }

      if (typeof data.design === "string" && data.design.trim()) {
        setDesign(data.design.trim());
        applied += 1;
      }
      ...
      if (typeof data.gsm === "number") {
        const gsmKey = gsmKeyFromNumber(data.gsm);
        if (gsmKey) {
          setGsm(gsmKey);
          applied += 1;
        }
      ...
      if (Array.isArray(data.materials) && data.materials.length) {
        ...
        for (const source of data.materials) {
          ...
          const localName = importedMaterialName(source.sourceName);
          if (!localName) continue;

          const materialId = materialIdByName(localName);
          if (!materialId) continue;

          byId.set(materialId, (byId.get(materialId) || 0) + source.percentage);
        }

        if (byId.size) {
          setMats(
            [...byId.entries()].map(([material_id, percentage]) => ({
              material_id,
      ...
      setImportMsg(
        applied > 1
          ? "מילאנו את הפרטים שמצאנו. אפשר לערוך הכול לפני הפרסום."
          : "הקישור נוסף. לא מצאנו בו פרטים נוספים שאפשר למלא בבטחה."
      );
    } catch (error: any) {
      setImportError(error?.message || "לא הצלחנו למלא פרטים מהקישור");
    } finally {
      setImportBusy(false);
    }
  }
  ...
  return (
    <div>
      <div className="section">
        <h2>מילוי פרטים מקישור</h2>

        <div className="field">
          <label htmlFor="wrap-import-url">קישור ל־WrapTrack</label>
          <input
            id="wrap-import-url"
            className="input"
            type="url"
            ...
            placeholder="https://wraptrack.org/Catalog/model/..."
          />
        </div>
        ...
        <button
          ...
          disabled={importBusy}
        >
          {importBusy ? "ממלאת פרטים…" : "מילוי פרטים מהקישור"}
        </button>
        ...
        {importError && (
          <div className="danger" role="alert" style={{ marginTop: 10 }}>
            {importError}
          </div>
        )}
      </div>
